...
class OSCPacker {
    ...
    _setupPackers() {
        ...
        this._packers['/midi/caftertouch'] = (args) => {
        ...
        // generated by Ableton live for MPE Pressure 
        this._packers['/midi/paftertouch'] = (args) => {
            if (!args || args.length < 3) return null;
            return {
                port : "",
                channel : args[0]?.value,
...
function oscServerFromConfig(config, wss) {
    ...
    if (config.buffer) {
        ...
        source.bypass('/midi/control');
        source.bypass('/midi/caftertouch');
        source.bypass('/midi/paftertouch');
        source.bypass('/midi/bend');
    } else {
